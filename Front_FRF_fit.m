...
labels.output='\delta\mathrm{L}_{\mathrm{tar}}';
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Load FRF fit
[G] = get_FRF_fit(Output,FitName,settings);
...
%% Evalute transferfunction
%evaluate transferfunction
w = logspace(-1,4,1000)*2*pi; % rads
[mag,phase,wout] = bode(G,w);
mag=reshape(mag,1,length(mag));
phase=reshape(phase,1,length(phase));
%plotting
hold on;
plot(h(1),wout/2/pi,mag,'-k','LineWidth',2); hold on;
plot(h(2),wout/2/pi,phase,'-k','LineWidth',2); hold on;
